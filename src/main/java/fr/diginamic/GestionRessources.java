...
public class GestionRessources {
    ...
    public void supprimerRessource(String type, int quantite) {
        Document ressource = collection.find(new Document("type", type)).first();
        if (ressource != null) {
            int nouvelleQuantite = ressource.getInteger("quantite", 0) - quantite;
            if (nouvelleQuantite >= 0) {
                mettreAJourRessource(type, nouvelleQuantite);
            } else {
                System.out.println("Quantité insuffisante pour : " + type);
            }
        } else {
            System.out.println("Ressource inexistante : " + type);
        }
    }
